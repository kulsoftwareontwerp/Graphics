package com.kuleuven.swop.group17.Graphics;
...
abstract class RecordingItem {
	...
	abstract void replay(int itemIndex, CanvasResource window);
}
class MouseEventItem extends RecordingItem {
	...
	@Override
	void replay(int itemIndex, CanvasResource window) {
...
class CanvasWindowRecording {
	...
	void replay(CanvasResource window) {
		int itemIndex = 0;
		for (RecordingItem item : items) {
...
public class CanvasResource {
	 ...
	 */
	protected CanvasResource(String title) {
		this.title = title;
	}
...
//		System.out.println(e);
	...
	BufferedImage captureImage() {
		...
		CanvasResource.this.paint(imageGraphics);
		return image;
	}
...
//			System.out.println("Painting...");
	...
	public static void replayRecording(String path, CanvasResource window) {
		try {
